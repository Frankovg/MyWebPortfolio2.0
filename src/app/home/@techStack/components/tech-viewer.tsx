...
function TechViewer() {
  ...
  return (
    <div className="w-full max-w-5xl flex max-sm:flex-col-reverse pt-16 pb-24">
      <div className="max-sm:w-full w-1/2 min-[930px]:w-3/5 px-12 grid grid-cols-3 min-[640px]:max-[799px]:grid-cols-2 min-[800px]:max-[929px]:grid-cols-3 min-[930px]:grid-cols-4 grid-rows-3 gap-8">
        {selectedCategoryObj && selectedCategoryObj.techs.map(tech => (
          <Image
            ...
            alt={`${tech.name} logo`}
            id={tech.name}
            className={`inline-block mr-2 transition-all duration-500 ease-in-out ${transitionClass}`}
            width={85}
            height={85}
          />
        ))}
